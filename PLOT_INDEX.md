...
### PLOT: beta_recovery_calibration
Name             : beta_recovery_calibration.pdf / beta_recovery_calibration.png
Output location  : plots/appendix_a/beta_recovery_calibration.pdf
                   plots/appendix_a/beta_recovery_calibration.png
Script name      : plot_beta_recovery_calibration.py
Script location  : plots/appendix_a/plot_beta_recovery_calibration.py
Data file(s)     : plots/appendix_a/results_table_A1_beta1.7_longchain.txt (beta=1.7 row)
                   plots/appendix_a/results_table_A1_corrected.txt (beta=3.0, beta=4.0 rows)
                   plots/appendix_a/data/beta1.7_N20_realization_medians.npy (N=20 ensemble,
                   beta=1.7, copied from code_backup/.../reproduction_2026-07-20/)
                   code_backup/.../reproduction_2026-07-20/beta3_N20_realization_medians.npy
                   code_backup/.../reproduction_2026-07-20/beta4_N20_realization_medians.npy
                   (N=20 ensembles, beta=3.0/4.0; generated by
                   plots/appendix_a/beta3_4_N20_ensemble/fit_beta34_N20_ensemble.py)
Contents         : Recovered vs. true beta, single panel, 1:1 reference line. Blue circles:
                   single-realization median + 5-95% CI (all three beta values, same numbers
                   as published Table A.1). Orange squares: N=20 realization-ensemble mean +/-
                   std, all three beta values.
Purpose          : The literal plot Johannes's Appendix A review comments 12-13 requested
                   (beta on y-axis, true value as reference, inferred value as error bar;
                   "several simulations (maybe 20)"), built after being identified as never
                   completed. Extended to beta=3/4 on 2026-07-25 to test whether their
                   single-realization miss is noise (like beta=1.7 was) or a persistent bias.
                   Finding: beta=4's miss is a statistically confirmed real bias (20/20
                   realizations below true, sign test p=0.000002); beta=3 is a weaker, more
                   ambiguous case (13/20 below, p=0.26, but t-test p=0.039). Full detail in
                   plots/appendix_a/README_beta_recovery_calibration.md and
                   plots/appendix_a/beta3_4_N20_ensemble/README.md.
Status           : Generated and reviewed 2026-07-25. NOT YET inserted into the paper.

---

### PLOT-SUPPORTING ANALYSIS: beta3_4_N20_ensemble
Location         : plots/appendix_a/beta3_4_N20_ensemble/
Scripts          : regenerate_beta3_N20_shortchain.py, regenerate_beta4_N20_shortchain.py,
                   fix_low_ess_beta34_N20.py, fit_beta34_N20_ensemble.py
Outputs          : beta3_N20_realization_medians.npy, beta4_N20_realization_medians.npy
                   (feed the beta_recovery_calibration plot above)
Purpose          : Self-contained, documented N=20 realization-ensemble test for beta=3/4
                   (T=1000, shift=7), mirroring the existing beta=1.7 ensemble, to test
                   whether Table A.1's beta=3/4 single-realization miss is noise or a
                   persistent bias. See README.md in that directory for full method,
                   statistical tests, and results.

---

### PLOT: beta_recovery_calibration_squares_only (SUPERSEDES beta_recovery_calibration)
Name             : beta_recovery_calibration_squares_only.pdf / .png
Output location  : plots/appendix_a/beta_recovery_calibration_squares_only.pdf
                   plots/appendix_a/beta_recovery_calibration_squares_only.png
Script name      : plot_beta_recovery_calibration_squares_only.py
Script location  : plots/appendix_a/plot_beta_recovery_calibration_squares_only.py
Data file(s)     : code_backup/.../reproduction_2026-07-20/beta1.7_N40_realization_medians.npy
                   code_backup/.../reproduction_2026-07-20/beta3_N40_realization_medians.npy
                   code_backup/.../reproduction_2026-07-20/beta4_N40_realization_medians.npy
                   code_backup/.../reproduction_2026-07-20/beta8_N40_realization_medians.npy
                   (all generated by fit_ensemble_N40_all_betas.py in that directory)
Contents         : Recovered vs. true beta, N=40 realization-ensemble mean +/- std only (single-
                   realization/Table A.1 markers deliberately dropped -- see README below), four
                   true beta values: 1.7, 3.0, 4.0, 8.0 (beta=8.0 is new).
Purpose          : Replaces the two-marker beta_recovery_calibration plot. (1) Single-realization
                   markers dropped because they are a Bayesian credible interval while the ensemble
                   markers are a frequentist-style interval -- not the same statistical object, not
                   meaningfully comparable on one axis. (2) beta=1.7/3.0/4.0 upgraded from N=20 to
                   N=40 (topped up with 20 further realizations each). (3) beta=8.0 added (N=40
                   fresh) to test whether the "flattening" trend visible in the N=20 three-point
                   version continues, saturates, or reverses at a much steeper input.
Key finding      : beta=8.0's recovered mean (3.30) is LOWER than beta=4.0's (3.54) despite the
                   true value being twice as large -- a reversal, not just a plateau. beta=4.0's
                   bias is now confirmed with overwhelming statistical force (40/40 realizations
