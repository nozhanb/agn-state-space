"""
Generate 20 independent colorednoise realizations at beta=3.0, T=1000,
shift=7 (same fixed configuration used for the published Table A.1 /
SNR-sweep beta=3 row), run each through the corrected AR(1)/HMC model
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "beta3_N20_shortchain_data")
...
flux_rng = numpy.random.default_rng(2026072510)
